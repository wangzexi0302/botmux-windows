...
export const messages: Record<string, string> = {
  ...
  'ai.shell.commands_are_shell.windows': 'This is native Windows. Run botmux.cmd from PATH using PowerShell; do not launch WSL or Linux Bash. If the CLI only has a Bash tool, use Windows node.exe and this checkout\'s dist/cli.js; do not pass Windows paths into WSL. These are shell commands, not MCP tools.',
  'ai.shell.how_to_send.windows': 'Reply using PowerShell: `botmux.cmd send --no-mention "message"`. Use --mention or --mention-back when needed. Attach images with --images and files with --files.',
  'ai.shell.multiline_heredoc.windows': 'Write multiline bodies to a UTF-8 file and use --content-file. Do not pass JSON.stringify / JSON-escaped text as a positional argument or literal `\\n` instead of newlines. Do not use Bash heredocs. PowerShell 5.1 default pipe encoding can damage Unicode.',
  'ai.shell.heredoc_example.windows': "Correct multiline example:\n```powershell\n$replyFile = Join-Path $env:TEMP ([guid]::NewGuid().ToString() + '.txt')\n$body = @'\nline 1\nline 2\n'@\n[IO.File]::WriteAllText($replyFile, $body, [Text.UTF8Encoding]::new($false))\nbotmux.cmd send --no-mention --content-file $replyFile\n```",
  'ai.shell.when_to_send.windows': 'Respond to messages addressed to you at least once with `botmux.cmd send` through PowerShell. print/echo does not send a reply. Only when the message is not for you, output just BOTMUX_NOTHING_TO_SEND as the final answer.',
