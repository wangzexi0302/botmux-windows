...
export const messages: Record<string, string> = {
  ...
  'ai.shell.commands_are_shell.windows': '这是原生 Windows 会话。botmux 命令已在 PATH 中，请用 PowerShell 执行 botmux.cmd，不要启动 WSL 或 Linux Bash。若 CLI 只提供 Bash 工具，使用 Windows node.exe 和本 checkout 的 dist/cli.js；不要把 Windows 路径交给 WSL。botmux 是 shell 命令，不是 MCP 工具。',
  'ai.shell.how_to_send.windows': '用 PowerShell 执行 `botmux.cmd send --no-mention "消息内容"` 回复用户；需要提及时改用 --mention 或 --mention-back。图片用 --images，文件用 --files。',
  'ai.shell.multiline_heredoc.windows': '多行正文请先写入 UTF-8 文件，再使用 --content-file。不要把 JSON.stringify / JSON 转义产生的字面量 \\n 作为正文，不要使用 Bash heredoc。PowerShell 5.1 的默认管道编码可能损坏中文。',
  'ai.shell.heredoc_example.windows': "正确多行示例：\n```powershell\n$replyFile = Join-Path $env:TEMP ([guid]::NewGuid().ToString() + '.txt')\n$body = @'\n第一行\n第二行\n'@\n[IO.File]::WriteAllText($replyFile, $body, [Text.UTF8Encoding]::new($false))\nbotmux.cmd send --no-mention --content-file $replyFile\n```",
  'ai.shell.when_to_send.windows': '发给你的消息至少用 `botmux.cmd send` 回应一次（通过 PowerShell 执行），不要用 print/echo 代替发送。只有消息不是发给你时，最终回答才仅输出 BOTMUX_NOTHING_TO_SEND。',
